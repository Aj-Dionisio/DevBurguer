...
class Sessioncontroller {
  async store(req, res) {
    const schema = Yup.object({
      /*Verificação se o email e senha está de acordo com os nossos parametros*/
      email: Yup.string()
        .email()
        .required() /*o email deve ser uma string, vai fazer a verificação se é um email e será obrigatório*/,
      password: Yup.string()
        .required()
        .min(6) /*Verifica se  é uma string e é obrigatório*/,
    });

    const isValid = await schema.isValid(req.body, {
      strict: true,
    }); /*método do schema para fazer a verificação*/
    ...
    const token = jwt.sign(
      { id: existUser.id, admin: existUser.admin, name: existUser.name },
